...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run Wandb Experiments for Task 2.4 of SMAI Assignment 3")
    
    parser.add_argument(
        '-p', '--path', default=f'{YAML_FOLDER}/task-2.4/single-layer-sgd-grid-search.yaml',
    ...
    advertisement_data_path = os.path.join(DATA_FOLDER, "advertisement.csv")
    X_train, X_val, X_test, y_train, y_val, y_test = get_and_process_advertisement_dataset(advertisement_data_path)
    ...
    # Starting Wandb
    wandb.login()
    project_name = sweep_configuration.get("project", "smai-assignment3-task2")
